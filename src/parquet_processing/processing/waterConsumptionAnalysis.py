...
def is_remplissage_WSU(df):
    for voiture in ['CAR01', 'CAR03', 'CAR05', 'CAR07']:
        ...
        for t in range(0, len(df)-1):
            if (df['WC_'+voiture+'_LCST_IWSUTANKLEVEL'][t] < df['WC_'+voiture+'_LCST_IWSUTANKLEVEL'][t+1]) and (remplissage_en_cours == 0) and (df['WC_'+voiture+'_LCST_IWSUTANKLEVEL'][
                for t in range(0, len(df)-1):
...
for rame, datas in dict_df.items():
    ...
    for mission in missions:
        # @ car mission variable externe a df
